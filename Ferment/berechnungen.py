...
def berechnung(t_ranges_array,param_array,const_array_array,phasen_anzahl):
	...
	for i in range(phasen_anzahl):
		...
		# Variable, um die letzte erfolgreiche Lösung zu speichern
		#last_sol = None  
		...
		if t_start != t_end:
			...
			#if( i == 0):
				# Optuna-Studie zur Maximierung von cx und Minimierung von cp
			study = optuna.create_study()
			study.optimize(lambda trial: objective(trial,t_start,t_end,y0,consts,t_span),n_trials=5)
			...
			if tmp_sol is not None and hasattr(tmp_sol, "success") and tmp_sol.success:
				...
				# Setze feed_S1 
				#consts[19] = best_params['feed_S1']
				sol = tmp_sol
				# Speichern der erfolgreichen Lösung für die nächste Iteration
				#last_sol = sol
				#y0[0] =  np.max(sol.y[0])  # Maximale Biomassekonzentration
				#y0[3] =  np.min(sol.y[3])  # Minimale Produktkonzentration
				cs_tmp = study.best_trial.user_attrs.get("cs",None)
				print('Optimierter Wert von cx fuer Phase ',i,' : ',  np.max(sol.y[0]))
				print('Optimierter Wert von cp fuer Phase ',i,' : ', np.min(sol.y[3]))
				print('feed_S1 fuer Phase ', i,' :',best_params['feed_S1'])
				print('bolusC cs fuer Phase ', i,' :',cs_tmp)
				ry = sol.y
				ry = ry.T
				rt = sol.t
